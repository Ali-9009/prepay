...
import DeviceUnlocked2Step from "../NewSim/Steps/DeviceUnlocked2Step";
import ConfirmServiceStep from "../NewSim/Steps/ConfirmServiceStep";
    ...
    <div>
      <div className="max-w-xl mx-auto grid grid-cols-2 gap-6 mb-6">
        <div className="flex flex-col gap-3">
          ...
          <div>
            <p>
              Use this option to start service later.
              Billing begins on the date you select.
            </p>
          </div>
        </div>
...
function NowStep1({ nextStep }) {
  return (
    <div>
      ...
      <div className="flex items-center flex-col md:flex-row justify-center mt-2 gap-2">
        <Button className="mt-2" text="I have my Hello Prepay SIM" onClick={nextStep} />
        <Button className="mt-2" text="I haven’t received my Hello Prepay SIM" onClick={nextStep} />
      </div>
    </div>
  ...
  return (
    <div className="max-w-md flex flex-col   mx-auto">
      <h3 className="text-lg mb-2 font-semibold">
        Do you have a Hello Prepay Mobile pSIM?
      </h3>
      <ul className=" space-y-3 text-sm">
        <li className="flex">
          <span className="w-2 h-2 bg-red-500 rounded-full mt-2 mr-3"></span>
          <div>
            <p className="font-semibold">If not, activate after you receive it or contact support  to arrange shipping.</p>
          </div>
        </li>
      </ul>
...
function StepperForm3() {
  ...
  const [postBranch, setPostBranch] = useState(null); // tracks branch from DeviceUnlocked2Step

  const nextStep = () => setCurrentStep((prev) => prev + 1);
  const prevStep = () => setCurrentStep((prev) => prev - 1);

  const commonSteps = [ConfirmServiceStep, DeviceUnlocked2Step];
  const futureSteps = [FutureStep1, FutureStep2, FutureStep3, FutureStep4, FutureStep5];
  const nowSteps = [NowStep1, NowStep2, NowStep3, NowStep4, NowStep5];

  // Build steps dynamically
  let steps = [...commonSteps];

  if (postBranch === "future") {
    steps = [...commonSteps, ...futureSteps];
  } else if (postBranch === "now") {
  ...
  return (
    <StepLayout step={currentStep} prevStep={prevStep}>
      <StepComponent
        ...
        prevStep={prevStep}
        setServiceType={
          StepComponent === ConfirmServiceStep ? setServiceType : undefined
        }
        setNextBranch={StepComponent === DeviceUnlocked2Step ? setPostBranch : undefined}
      />
    </StepLayout>
  );
}
